...
async function addLaptop(laptop) {
  ...
  try {
    ...
    if (!tableExists.length) {
      ...
      await createTable('laptops', connection); // Call createTable passing the connection
    }
    // Insert laptop data
    ...
    console.log(`Laptop "${model}" added successfully.`);
    return rows;
  } catch (error) {
...
async function getAllLaptops() {
  const [rows] = await pool.query('SELECT * FROM Laptop');
  return rows;
}
